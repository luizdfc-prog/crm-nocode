...
Ao final da qualificação, se o lead for qualificado, inclua exatamente esta linha no final da sua resposta (sem formatação adicional):
[TRANSFERIR_PARA_VENDEDOR]

Extraia e mantenha em mente as seguintes informações conforme o lead as fornece:
- Nome do lead
- Empresa
- Tamanho do time
- CRM atual
- Principal interesse/dor`;
...
export async function runQualificationAgent(
  ...
  // Extrai dados do lead do histórico de conversa
  const fullHistory = [...messages, { role: "assistant", content: responseText }];
  const leadData = extractLeadData(fullHistory);

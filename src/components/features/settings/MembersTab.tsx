...
export function MembersTab({
  ...
  const loadData = useCallback(async () => {
    ...
    // Buscar membros e profiles em queries separadas para evitar
    // ambiguidade no formato do join (objeto vs array)
  ...
  async function removeMember(memberId: string) {
    ...
    const { error } = await supabase
      ...
      .eq("id", memberId)
    if (error) {
      console.error("Erro ao remover membro:", error.message, error.code)
    ...
    <div className="flex gap-6">
      ...
      <div className="flex items-start justify-between gap-4">
        <div>
          ...
          <p className="mt-0.5 text-sm text-pf-text-muted">
            {memberCount}{" "}
            {isFreePlan ? `/ ${FREE_LIMIT} — plano Free` : "membros"}
          </p>
        </div>
        {isAdmin && (
          <button
      ...
      {isAdmin && atLimit && (
        <div className="rounded-xl border border-pf-warm/30 bg-pf-warm/10 px-4 py-3 text-sm text-pf-warm">
          Limite de {FREE_LIMIT} membros atingido no plano Free.{" "}
          <span className="font-medium">Faça upgrade para Pro</span> para
          adicionar membros ilimitados.
        </div>
      )}
